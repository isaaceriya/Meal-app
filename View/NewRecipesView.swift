...
struct NewRecipesView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            Text("New Recipes")
                .font(.system(size: 27, weight: .bold)) // shows the new recipe text
            ...
            VStack {
                ForEach(viewModel.items ?? viewModel.placeholders, id: \.idMeal) { item in
                    NavigationLink(
                        ...
                        label: {
                            NewRecipeView(item: item) // When a recipe is clicked It connects to the API to show the details
                        })
                }
            }
        }
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(spacing: 4) {
                    Image(systemName: "clock")
                        .font(.system(size: 13))
                        .foregroundColor(.gray) // This shows a clock
                    
                    Text("10 minutes")
                        .font(.system(size: 13, weight: .semibold))
                        .foregroundColor(.gray)
                        .redacted(reason: item.strCategory == nil ? .placeholder : .init()) // This shows how long the recipes take
                }
            }
            .padding(.top)
            .padding(.leading)
